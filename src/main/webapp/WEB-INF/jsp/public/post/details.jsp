...
<body>
	<div class="container">
		...
		<a href="/public"><button type="button"
				class="btn btn-default btn-sm">
				<span class="glyphicon glyphicon-home"></span> Home
			</button></a>
		<div id="divPlay" class="col-md-6 thumb">

			<a id="img-href" class="image-popup-vertical-fit thumb" href="${post.filename}"
				title="${post.title}"> <video id="video-control" width="320" height="240" poster="${post.filename}" controls autoplay> <source id="containt-source" src="${post.filename}"
					type="video/mp4">
				Your browser does not support the video tag. </video> 
				<%-- <img class="img-responsive details"
				src="${post.filename}"> --%>
			</a>
		</div>

		<div class="col-md-6 columns">
		<span>@Creation date : ${fn:substring(post.creationTime, 0, 10)}<br>
		@Author: ${post.user.firstName} ${post.user.lastName}</span>
			<h2>Description</h2>
			<p>${post.description}</p>
			<h2>Votes</h2>
			<sec:authorize access="isAuthenticated()">
				<button type="button" id="btnVotePlus" onclick="votePlus()"
					class="btn btn-success btn-lg"
					<c:if test="${not empty userVote and userVote == true}"> disabled</c:if>>
					<span class="glyphicon glyphicon-thumbs-up"></span><span
						id="votePlus">${voteplus}</span> Like
				</button>
				<button type="button" id="btnVoteMoins" onclick="voteMoins()"
					class="btn btn-danger btn-lg"
					<c:if test="${not empty userVote and userVote == false}"> disabled</c:if>>
			...
			<div class="col-lg-8 col-sm-8 col-xs-12">
				<ul class="media-list comments">
					<li class="media"><a class="pull-left" href="#"> <img
							...
							src="${comment.user.filename}" width="64"
...
<sec:authorize access="isAuthenticated()">
	<div class="col-md-10">
		...
		<div class="status-upload">
			<form action="${pageContext.request.contextPath}/user/post/comment"
				method="post">
				<textarea name="comment" placeholder="What are you doing right now?"></textarea>
				<input type="hidden" name="${_csrf.parameterName}"
					value="${_csrf.token}" /> <input type="hidden" name="id_post"
					value="${post.id}" />
...
<sec:authorize access="isAnonymous()">
<div class="col-md-10">
<p>Please login if you want comment this post.</p>
</div>
</sec:authorize>
	</div>
	</div>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			var re = /(?:\.([^.]+))?$/;
			var filename = $('#containt-source').attr("src");
			...
			case 'bmp':
				...
				$('#video-control').removeAttr('autoplay');
				$('#containt-source').remove();
				break;
			default:
				$('#video-control').removeAttr('poster');
				$('#divPlay').removeClass('thumb');
				$('#img-href').removeAttr('href');
				break;
			}
	...
	<script type="text/javascript">
		function votePlus() {
			...
			$("#btnVoteMoins").removeAttr("disabled", "disabled").button(
					'refresh');
		}
		...
		function voteMoins() {
			...
			$("#btnVotePlus").removeAttr("disabled", "disabled").button(
					'refresh');
		}
	</script>
</body>
